...
The repository is migrating away from `src/international-space-bar/`. That tree is certain to be decommissioned. The exact target structure is still evolving, so the README migration note and this design's migration details should be reviewed before implementation work.

Current expected migration targets:
