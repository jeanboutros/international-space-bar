...
| **Prettier**                  | Formatting (owns import organisation, auto-fix)                                                       |
...
> **`pnpm check` must exit with code 0 after every code change.** It runs Prettier (format, auto-fix) followed by ESLint (type-aware, auto-fix). If either reports unfixable errors, resolve them manually before proceeding.
...
src/international-space-bar-server/
  ...
  graphs/        # Server-wired LangGraph workflows
