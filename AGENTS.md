...
src/international-space-bar-server/
  ...
  openresponses/ # OpenResponses protocol — controller, service, schemas
    blocks/      # Block factories (messageBlock, reasoningBlock, functionCallBlock)
...
- **Documentation must be referenced like an academic paper.** Every claim about an external library, framework, protocol, or API must link to a working, fetched-at-write-time source — official docs, official spec, official API reference, or a versioned source-controlled artefact (e.g. a permalinked GitHub blob). Never cite a URL from memory or training data. Never paraphrase API behaviour without a link to the page that documents it. Before merging any document, every external link in it must be opened and confirmed to load the cited content; broken or redirected links are blockers, not warnings. Internal cross-references (`docs/foo.md#section`) follow the same rule — broken anchors are bugs. When a fact's source cannot be verified, write "unverified" next to the claim or omit the claim; do not paper over the gap with a plausible-sounding URL.
